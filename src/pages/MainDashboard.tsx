                ...
                {properties.map((prop) => {
                  ...
                  return ((isOwner || isCoOwner) && prop.isRental) ? (
                    ...
                    <Paper key={prop.id} sx={{ borderRadius: 2, width: isSingleProperty ? { xs: '100%', sm: '100%', md: singleCardWidth } : { xs: mobileCardWidthXs, sm: cardWidthSm, md: cardWidthMd }, minWidth: isSingleProperty ? { xs: '100%', sm: '100%', md: singleCardWidth } : { xs: mobileCardWidthXs, sm: cardWidthSm, md: cardWidthMd }, maxWidth: isSingleProperty ? { xs: '100%', sm: '100%', md: singleCardWidth } : { xs: mobileCardWidthXs, sm: cardWidthSm, md: cardWidthMd }, height: 'auto', overflow: 'hidden', position: 'relative', display: 'flex', flexDirection: 'column', flex: '0 0 auto', p: 2, boxShadow: '0 2px 10px rgba(0,0,0,0.16)', ...cardSnapSx, ...mobileCardContentSx }}>
                      ...
                      {/* Main content wrapper - 2x2 grid for single property, vertical for multiple */}
                      {isSingleProperty ? (
                        // SINGLE PROPERTY: 2x2 grid layout for Homeowner
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, height: 580 }}>
                          {/* Top Row: Image (50%) + Property Breakdown (50%) - Height 52% */}
                          <Box sx={{ display: 'flex', gap: 2, flex: '0 0 52%', overflow: 'hidden' }}>
                            {/* Top-Left: Image */}
                            ...
                            <Box sx={{ width: { xs: '100%', md: '50%' }, display: 'flex', flexDirection: 'column', gap: 0.8, p: 2 }}>
                              ...
                              <Typography sx={{ fontSize: 14, color: '#333', fontFamily: 'Nunito, Arial, sans-serif', lineHeight: 1.7 }}>
                                <strong>Property Value:</strong> {formatCurrency(propertyValue)}<br/>
                                <strong>Equity:</strong> {formatCurrency(equity)}<br/>
                                <strong>Appreciation:</strong> {appreciation !== 0 ? formatCurrency(appreciation) : ''}<br/>
                                <strong>Expenses:</strong> {(() => {
                                  const currentState = cardButtonState[prop.id] || 'Monthly';
                                  const monthlyExact = expenses;
                                  return currentState === 'Yearly'
                              ...
                              <Typography sx={{ fontSize: 14, color: '#333', fontFamily: 'Nunito, Arial, sans-serif', lineHeight: 1.7 }}>
                                ...
                                {!prop.noMortgage && (
                                  <>
                                    <strong>Remaining Balance:</strong> {remainingBalance < 0 ? '-' : ''}${Math.abs(remainingBalance).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                  </>
                                )}
                              </Typography>
                                ...
                                {(() => {
                                  if (prop.noMortgage || (!prop.term && !prop.date)) {
                                    return (
                                      <>
                                        <Box sx={{ width: '100%', height: 18, bgcolor: '#e8e8e8', borderRadius: 10, overflow: 'hidden', display: 'flex', justifyContent: 'flex-end' }}>
                                          <Box sx={{ height: '100%', width: '0%', bgcolor: '#89AE99', borderRadius: 10 }} />
                                        </Box>
                                        <Typography sx={{ fontSize: 12, color: '#666', fontFamily: 'Nunito, Arial, sans-serif', mt: 0.5, textAlign: 'center', fontWeight: 500 }}>
                                          0 Months Left
                                        </Typography>
                                  ...
                                  return (
                                    <>
                                      <Box sx={{ width: '100%', height: 18, bgcolor: '#e8e8e8', borderRadius: 10, overflow: 'hidden', display: 'flex', justifyContent: 'flex-end' }}>
                                        <Box sx={{ height: '100%', width: `${Math.min(100, remainingPercentage)}%`, bgcolor: '#89AE99', borderRadius: 10 }} />
                                      </Box>
                                      <Typography sx={{ fontSize: 12, color: '#666', fontFamily: 'Nunito, Arial, sans-serif', mt: 0.5, textAlign: 'center', fontWeight: 500 }}>
                          ...
                          {/* Bottom Row: Chart + Tasks — column on xs/sm, row on md */}
                          <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 2, flex: { xs: '0 0 auto', md: '0 0 48%' }, alignItems: 'center' }}>
                            {/* Chart: full width on mobile, 50% on desktop */}
                            <Box sx={{ width: { xs: '100%', md: '50%' }, display: 'flex', alignItems: 'center', justifyContent: 'center', mt: { xs: 0, md: -1.5 } }}>
                              <Box sx={{ position: 'relative', width: 300, height: 300 }}>
                                <svg width="300" height="300" viewBox="0 0 110 110" style={{ position: 'absolute' }}>
                                  ...
                                  {(() => {
                                    ...
                                    if (prop.isPT && !prop.noMortgage) {
                                      ...
                                      insuranceAmount += (propertyTaxAmount / 12);
                                    }
                                    let hoaAmount = prop.hoa ? (prop.yearly === 'Yearly' ? parseValue(prop.hoa) / 12 : parseValue(prop.hoa)) : 0;
                                    let pmiAmount = parseValue(prop.pmi);
                                    if (prop.noMortgage) {
                                  ...
                                  {(() => {
                                    ...
                                    // Monthly total for 12-segment tooltips
                                    ...
                                    // Monthly/Yearly state: Paid vs Owed segments
                                    const purchasePriceVal = prop.price ? parseFloat(prop.price as any) : 0;
                                    const remainingBalanceVal = prop.noMortgage ? 0 : (prop.balance ? parseFloat(prop.balance as any) : 0);
                                    const equityVal = purchasePriceVal - remainingBalanceVal;
                                    let filledPercentage = 0;
                                    if (prop.noMortgage) {
                                      filledPercentage = 100;
                                    } else if (purchasePriceVal > 0) {
                                      filledPercentage = Math.min(100, Math.max(0, (equityVal / purchasePriceVal) * 100));
                                    }
                                    const dashLength = (filledPercentage / 100) * circ;
                                    const dashGap = circ - dashLength;
                                    ...
                                    return (
                                      <>
                                        ...
                                        {showSegments && Array.from({ length: segmentCount }).map((_, i) => {
                                          ...
                                          return (
                                            <circle
                                              key={`shrink-segment-homeowner-single-${i}`}
                                              cx="55" cy="55" r={r}
                                              fill="none"
                                        ...
                                        {/* Background grey circle (Owed) - hide during shrink/expand so segments show through */}
                                        {!showSegments && (
                                          <circle cx="55" cy="55" r={r} fill="none" stroke="#e8e8e8" strokeWidth="6.5"
                                            style={{
                                            ...
                                            onMouseEnter={e => {
                                              setHoveredSegment({
                                                label: 'Owed',
                                                amount: Math.max(0, remainingBalanceVal),
                                                percent: purchasePriceVal > 0 ? (Math.max(0, remainingBalanceVal) / purchasePriceVal) * 100 : 0,
                                                period: 'Monthly',
                                                x: e.clientX,
                                                y: e.clientY,
                                        ...
                                        {filledPercentage > 0 && (
                                          <circle
                                            key={`paid-circle-homeowner-single-${rentalAnimationKey[prop.id] || 0}`}
                                            cx="55" cy="55" r={r} fill="none" stroke="#6A7F91" strokeWidth="6.5"
                                            strokeDasharray={animState === 'to5' ? `${targetLength} ${targetGap}` : animState === 'toFull' || animState === 'shrink' ? `${circ} 0` : animState === 'expandFromZero' ? `0 ${circ}` : animState === 'shrinkToOrigin' ? `${circ} 0` : `${dashLength} ${dashGap}`}
                                            strokeDashoffset={-circ / 4}
                                            strokeLinecap="round"
                                            ...
                                            onMouseEnter={e => {
                                              setHoveredSegment({
                                                label: 'Paid',
                                                amount: Math.max(0, equityVal),
                                                period: 'Monthly',
                                                percent: purchasePriceVal > 0 ? (Math.max(0, equityVal) / purchasePriceVal) * 100 : 0,
                                                x: e.clientX,
                                                y: e.clientY,
                                                source: 'homeowner-inner',
                                                propertyId: prop.id,
                                  ...
                                  {!prop.noMortgage && (cardButtonState[prop.id] || 'Monthly') !== 'Actual' && (
                                    <text fill="#999" fontSize="6" textAnchor="middle">
                                      <textPath href="#arcPathHomeownerSingle" startOffset="50%">
                                        {(() => {
                                          const purchasePriceVal = prop.price ? parseFloat(prop.price as any) : 0;
                                          const remainingBalanceVal = prop.balance ? parseFloat(prop.balance as any) : 0;
                                          const equityVal = purchasePriceVal - remainingBalanceVal;
                                ...
                                {(() => {
                                  ...
                                  return (
                                    <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', textAlign: 'center', zIndex: 1, pointerEvents: canEdit ? 'auto' : 'none', cursor: canEdit ? 'pointer' : 'default' }}
                                      onClick={canEdit ? () => handleOpenEditModal(prop, 'owner') : undefined}
                                    >
                                      <Typography sx={{ fontSize: 26, fontWeight: 700, color: showNA ? '#89AE99' : '#89AE99', fontFamily: 'Nunito, Arial, sans-serif', '&:hover': canEdit ? { textDecoration: 'underline' } : {} }}>
                                        {showNA ? 'Add info' : ((monthlyExact < 0 ? '-' : '') + `$${Math.abs(monthlyExact).toLocaleString('en-US')}`)}
                                      </Typography>
                                      <Typography sx={{ fontSize: 12, color: '#888', fontFamily: 'Nunito, Arial, sans-serif', lineHeight: 1 }}>
                                ...
                                {/* Tooltips */}
                                <SegmentTooltip data={hoveredSegment} source="homeowner" propertyId={prop.id} variant="full" />
                        ...
                        // MULTIPLE PROPERTIES: Original vertical layout for Homeowner
                        <>
                      {/* Property Image Section */}
                      <Box sx={{ position: 'relative', height: { xs: 200, sm: 240, md: 290 }, overflow: 'hidden', cursor: 'pointer', flexShrink: 0, borderRadius: 2 }}
                        onClick={() => {
                      ...
                      {/* Property Breakdown Section for Homeowner */}
                      <Box sx={{ p: 1.5, pb: 0, flex: 1, display: 'flex', flexDirection: 'column', gap: 0.8, overflowY: 'auto' }}>
                        {/* Breakdown with Gauge and Details */}
                        <Box sx={{ display: 'flex', gap: 2, alignItems: 'flex-start' }}>
                          {/* Circular Gauge with Double Circle - Same as Rental Card */}
                          <Box sx={{ flexShrink: 0, width: 220, height: 220, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', mt: -1.5, ml: -1 }}>
                            <svg width="220" height="220" viewBox="0 0 110 110" style={{ position: 'absolute' }}>
                              <defs>
                                <path
                                  id="arcPathHomeowner"
                              ...
                              {(() => {
                                ...
                                const forecastingMonthly = calculateForecastingMonthly(prop.forecasting);
                                const total = mortgagePayment + insuranceAmount + hoaAmount + pmiAmount + forecastingMonthly;
                                // Create array of segments with only non-zero values
                                const segments = [
                                  ...(prop.noMortgage ? [] : [{ value: mortgagePayment, color: '#E35E61', label: 'Mortgage' }]),
                                  { value: insuranceAmount, color: '#EEB05E', label: 'Taxes/Insurance' },
                                  { value: hoaAmount, color: '#db83ad', label: 'HOA' },
                                ...
                                const gap = 8;
                                const radius = 44;
                                const circumference = 2 * Math.PI * radius; // ~276.46 units (correct circumference)
                                let totalGaps = drawSegments.length > 1 ? gap * drawSegments.length : 0;
                                let availableSpace = circumference - totalGaps;
                                // Calculate rotation angle to center the biggest segment at 9 o'clock
                                const biggestSegment = drawSegments[0];
                                const biggestPercent = (biggestSegment.value / total) * 100;
                                const biggestDashLength = (biggestPercent / 100) * availableSpace;
                                const angleForBiggest = (biggestDashLength / circumference) * 360;
                                const rotationAngle = 180 - (angleForBiggest / 2); // Center at 9 o'clock
                                let offset = 0;
                                const circles: React.ReactElement[] = [];
                                drawSegments.forEach((segment) => {
                                  const percent = (segment.value / total) * 100;
                                ...
                                return (
                                  <>
                                    ...
                                    {showSegments && Array.from({ length: segmentCount }).map((_, i) => {
                                      ...
                                      return (
                                        <circle
                                          key={`shrink-segment-homeowner-multi-${i}`}
                                          cx="55" cy="55" r={r}
                                          fill="none"
                                          stroke={isInRange ? "#6A7F91" : "#e0e0e0"}
                                          strokeWidth="6.5"
                                    ...
                                    {filledPercentage > 0 && (
                                      <circle
                                        key={`paid-circle-homeowner-multi-${rentalAnimationKey[prop.id] || 0}`}
                                        cx="55"
                                        cy="55"
                                        r={r}
                                        fill="none"
                              ...
                              {!prop.noMortgage && (cardButtonState[prop.id] || 'Monthly') !== 'Actual' && (
                                <text fill="#999" fontSize="6" textAnchor="middle">
                                  <textPath href="#arcPathHomeowner" startOffset="50%">
                                    {(() => {
                                      const purchasePrice = prop.price ? parseFloat(prop.price as any) : 0;
                                      const remainingBalance = prop.balance ? parseFloat(prop.balance as any) : 0;
                                      const equity = purchasePrice - remainingBalance;
                            ...
                            {(() => {
                              ...
                              return (
                                <Box sx={{ textAlign: 'center', zIndex: 1, pointerEvents: canEdit ? 'auto' : 'none', cursor: canEdit ? 'pointer' : 'default' }}
                                  onClick={canEdit ? () => handleOpenEditModal(prop, 'owner') : undefined}
                                >
                                  <Typography sx={{ fontSize: 22, fontWeight: 700, color: showNA ? '#89AE99' : '#89AE99', fontFamily: 'Nunito, Arial, sans-serif', '&:hover': canEdit ? { textDecoration: 'underline' } : {} }}>
                                    {showNA ? 'Add info' : ((monthlyExact < 0 ? '-' : '') + `$${Math.abs(monthlyExact).toLocaleString('en-US')}`)}
                                  </Typography>
                                  <Typography sx={{ fontSize: 11, color: '#888', fontFamily: 'Nunito, Arial, sans-serif', lineHeight: 1 }}>
                                    {cardButtonState[prop.id] === 'Yearly' ? 'Yearly Expenses' : cardButtonState[prop.id] === 'Actual' ? 'Year to Date' : 'Monthly Expenses'}
                          ...
                          <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 0.8 }}>
                            ...
                            <Typography sx={{ fontSize: 14, color: '#333', fontFamily: 'Nunito, Arial, sans-serif', lineHeight: 1.5 }}>
                              <span style={{ fontWeight: 700 }}>Property Value:</span> {formatCurrency(propertyValue)}<br/>
                              <span style={{ fontWeight: 700 }}>Equity:</span> {formatCurrency(equity)}<br/>
                              <span style={{ fontWeight: 700 }}>Appreciation:</span> {appreciation !== 0 ? formatCurrency(appreciation) : ''}<br/>
                            ...
                            <Typography sx={{ fontSize: 14, color: '#333', fontFamily: 'Nunito, Arial, sans-serif', lineHeight: 1.5, mt: prop.noMortgage ? 1.8 : 0.8, mb: prop.noMortgage ? 1.5 : 0 }}>
                              <span style={{ fontWeight: 700 }}>Paid-off date:</span> {(() => {
                                // If property is marked as paid off, use the mortgagePaidOffDate
                                if (prop.noMortgage) {
                                  return prop.mortgagePaidOffDate ? new Date(prop.mortgagePaidOffDate).toLocaleDateString('en-US', { month: 'long', year: 'numeric' }) : '';
                                }
                                // Otherwise, calculate from purchase date + term
                                if (prop.date) {
                                  try {
                                    const purchaseDate = new Date(prop.date);
                                    const term = prop.term || '';
                              ...
                              {!prop.noMortgage && (
                                <>
                                  <span style={{ fontWeight: 700 }}>Remaining Balance:</span> {remainingBalance < 0 ? '-' : ''}${Math.abs(remainingBalance).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </>
                              )}
                            </Typography>
                            
                            {/* Progress Bar for Paid Off */}
                            <Box sx={{ mt: 1 }}>
                              {(() => {
                                // If property is paid off or no mortgage data entered, show 0 months left
                                if (prop.noMortgage || (!prop.term && !prop.date)) {
                                  return (
                                    <>
                                      <Box sx={{ width: '100%', height: 15, bgcolor: '#e8e8e8', borderRadius: 10, overflow: 'hidden', display: 'flex', justifyContent: 'flex-end' }}>
                                ...
                                if (prop.date) {
                                  try {
                                    ...
                                    const today = new Date();
                                    monthsElapsed = (today.getFullYear() - purchaseDate.getFullYear()) * 12 + (today.getMonth() - purchaseDate.getMonth());
                                    monthsElapsed = Math.max(0, monthsElapsed); // Ensure non-negative
                                  } catch (e) {
                                    monthsElapsed = 0;
                                  }
                                }
      ...
      <Box sx={{ position: 'relative', mb: 3 }}>
        ...
        >
                  ...
                  {/* Property Breakdown - 55% width, left aligned, vertical center, improved for cutting issue */}
                    <Box sx={{ width: '55%', minWidth: 240, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', mt: 0, height: 300, minHeight: 300, pb: 4, boxSizing: 'border-box' }}>
                      <Box sx={{ width: '100%', maxWidth: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', height: '100%', minHeight: '100%', overflow: 'visible', boxSizing: 'border-box' }}>
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.7, wordBreak: 'break-word', whiteSpace: 'normal', width: '100%' }}>
                          {[
                          ...
                          .map((item, index) => (
                            <Box key={index} sx={{ mb: -0.5, display: 'block', maxWidth: '100%', overflowWrap: 'anywhere', wordBreak: 'break-word', width: '100%' }}>
                              <Box sx={{ mr: 0.5, width: 10, height: 10, borderRadius: '50%', bgcolor: item.color, flexShrink: 0, display: 'inline-block', verticalAlign: 'middle' }} />
                              <Typography sx={{ fontSize: 14, fontWeight: 550, color: '#333', fontFamily: 'Nunito, Arial, sans-serif', display: 'inline', maxWidth: '90%', overflowWrap: 'anywhere', wordBreak: 'break-word', width: 'auto' }}>
                                {item.label}: <span style={{ fontSize: 14, fontWeight: 400 }}>{formatCurrency(Math.round(item.value), 0)}</span>
                              </Typography>
                            </Box>
                          ))}
                        </Box>
              ...
              return (
                <Box data-board sx={{ bgcolor: '#fff', borderRadius: 2, py: 1.5, px: 4, boxShadow: '0 2px 8px rgba(0,0,0,0.1)', minWidth: boardMinWidth, position: 'relative', display: 'flex', flexDirection: 'column', gap: 2, flex: boardFlexValue }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
                    {/* Task List - 50% width */}
                    <Box sx={{ width: '50%', display: 'flex', flexDirection: 'column', gap: 2, mt: 2, mb: 2 }}>
                      {(() => {
                    ...
                    {/* Chart - 50% width */}
                    <Box sx={{ width: '50%', minWidth: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <Box sx={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'visible', mt: 3, mb: 3 }}>
                        {(() => {
                          const totalTasks = 4;
                          ...
                          return (
                            <svg width="180" height="180" viewBox="0 0 110 110" style={{ display: 'block', overflow: 'visible' }}>
                              {((percent === null || percent === undefined || isNaN(percent))) ? (
                                <circle cx="55" cy="55" r="55" fill="none" stroke="#e8e8e8" strokeWidth="12" />
                              ) : percent < 1 ? (
                                <>
                                  <circle cx="55" cy="55" r="55" fill="none" stroke="#e8e8e8" strokeWidth="12" />
                                  <circle
            ...
            return (
              <Box data-board sx={{ bgcolor: '#fff', borderRadius: 2, py: 1.5, px: 3, boxShadow: '0 2px 8px rgba(0,0,0,0.1)', minWidth: boardMinWidth, flex: boardFlexValue }}>
                ...
                <Box sx={{ display: 'flex', gap: 2 }}>
                  ...
                  {/* Bar Chart using Recharts - 50% width */}
                  <Box 
                    ref={chartContainerRef}
                    sx={{ width: '50%', height: 220, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', '& svg:focus': { outline: 'none' }, '& *:focus': { outline: 'none' }, '& rect:focus': { outline: 'none' } }}
                  >
          ...
          <Box data-board sx={{ bgcolor: '#fff', borderRadius: 2, py: 1.5, px: 4, boxShadow: '0 2px 8px rgba(0,0,0,0.1)', minWidth: boardMinWidth, position: 'relative', display: 'flex', flexDirection: 'column', gap: 2, flex: boardFlexValue }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: { xs: 2, sm: 3, md: 4 }, alignItems: { xs: 'center', md: 'flex-start' }, justifyContent: 'center', width: '100%' }}>
              ...
              {(() => {
                ...
                return (
                  <Box sx={{ mt: -1, position: 'relative', width: { xs: 200, sm: 220, md: 240 }, height: { xs: 200, sm: 220, md: 240 }, minWidth: { xs: 200, sm: 220, md: 240 }, minHeight: { xs: 200, sm: 220, md: 240 }, display: 'flex', alignItems: 'center', justifyContent: 'center', mx: 'auto', flexShrink: 0 }}>
                    <svg width="240" height="240" viewBox="0 0 110 110" style={{ position: 'absolute' }}>
                    ...
                    <Box sx={{ position: 'absolute', left: 0, top: 0, width: 240, height: 240, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
              ...
              <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 1, minWidth: { xs: 280, sm: 200 }, maxWidth: { xs: 'none', sm: 320, md: 340 }, alignItems: { xs: 'center', md: 'flex-start' }, justifyContent: 'center', overflow: 'hidden' }}>
          ...
          {(() => {
            ...
            return (
              <Box data-board sx={{ bgcolor: '#fff', borderRadius: 2, py: 1.5, px: 3, boxShadow: '0 2px 8px rgba(0,0,0,0.1)', minWidth: boardMinWidth, flex: boardFlexValue }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                  <Typography sx={{ fontSize: 20, fontWeight: 700, color: '#222', fontFamily: 'Nunito, Arial, sans-serif' }}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5, py: 2 }}>
                  {sharedMembers.map((item, index) => (
                    <Box key={index} sx={{ display: 'flex', alignItems: 'center', gap: 2, width: '100%' }}>
                      <Box
                      ...
                      <Typography sx={{ fontSize: 20, fontWeight: 400, color: '#222', fontFamily: 'Nunito, Arial, sans-serif', flex: 1 }}>
                      ...
                      <Typography
                        sx={{ fontSize: 20, color: '#888', fontFamily: 'Nunito, Arial, sans-serif', fontWeight: 400, textAlign: 'right', minWidth: 120 }}
                        onMouseEnter={async (e:any) => {
                          const clientX = e.clientX;
                          const clientY = e.clientY;
                          const uid = (item as any).userId;
